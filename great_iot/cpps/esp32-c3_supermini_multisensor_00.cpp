...
struct BatchPacket {
  MeasurementRow rows[MAX_BATCH_ROWS];
  int count;
};
...
void setup() {
  ...
  Serial.printf("Measurement interval: %ld ms, Logging interval: %ld ms, Batch size: %d\n",
                measurement_interval, logging_interval, BATCH_SIZE);
  ...
  BATCH_SIZE = logging_interval / measurement_interval;
  if (BATCH_SIZE < 1) BATCH_SIZE = 1;
  if (BATCH_SIZE > MAX_BATCH_ROWS) BATCH_SIZE = MAX_BATCH_ROWS;
  Serial.printf("Batch size: %d\n", BATCH_SIZE);

  xTaskCreatePinnedToCore(MeasurementTask, "MeasureTask", 8192, NULL, 2, &MeasurementTaskHandle, 0);
  xTaskCreatePinnedToCore(LoggingTask, "LogTask", 8192, NULL, 1, &LoggingTaskHandle, 0);

  Serial.println("--- FreeRTOS Threads Initialized ---");
}
...
void LoggingTask(void *parameter) {
  ...
  while(1) {
    ...
    if (xQueueReceive(dataQueue, &receivedRow, pdMS_TO_TICKS(logging_interval)) == pdPASS) {
      currentBatch.rows[currentBatch.count] = receivedRow;
      currentBatch.count++;
      ...
      if (currentBatch.count > 0) {
        Serial.printf("[Flush] %d rows queued, sending now.\n", currentBatch.count);
        bool success = transmitBatch(currentBatch);
        if (!success) {
          Serial.println("[Flush] Send failed, will retry.");
        }
        currentBatch.count = 0;
      }
    }
    ...
    if (backupCount > 0 && WiFi.status() == WL_CONNECTED) {
      Serial.printf("[Recovery] Syncing %d cached packets...\n", backupCount);
      ...
      while (backupCount > 0 && WiFi.status() == WL_CONNECTED) {
        if (transmitBatch(backupBuffer[backupTail])) {
          ...
          Serial.println("[Recovery] Transmission failed mid-sync.");
          break; 
        }
      }
    }
